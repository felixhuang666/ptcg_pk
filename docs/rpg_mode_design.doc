...
- **Header Navigation**: Contains actions like 'Back', 'Refresh', 'Settings', and 'Toggle Chat'.
...
- **Status Overlay**: Lightweight overlay providing current coordinates, map name, and zoom level.
...
- `PhaserGame` (internal to `RpgMode.tsx`) handles map rendering, sprite animations, inputs, and the camera.
...
- Serves static assets (sprites) and map data via REST endpoints (`/api/map`, `/api/maps`).
...
- **State Synchronization**: Position and animations are synchronized locally in memory on the server during the session.

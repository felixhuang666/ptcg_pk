...
*   **Resolution**: Game Objects support has been added to the RpgSceneEditor, but scripting, node-based dialog trees, and advanced physics integration are still lacking. These aspirational features have been logged in `docs/TODO.md`. The design document currently overstates the capabilities of the editor. Future development phases will need to systematically implement these advanced features or the design spec must be scaled down to reflect MVP status.
...
*   **Visibility Toggle**: (Planned feature) The editor may need tools to hide specific layers (like the `topLayer`) to make editing underlying tiles easier.
...
## 6. Current Todos
*   Unify and clearly document the 0-based vs 1-based tile indexing logic across tileset JSON generation, backend processing, and frontend Phaser array usage to prevent offset bugs.
*   Implement functional event handling logic for the `objectEvent` layer in the main `RpgMode` gameplay loop.
*   Enhance the editor UI to allow hiding/showing individual layers during the editing process.
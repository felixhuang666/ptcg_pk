...
interface PlayerCameraCaptureProps {
  userId: string;
  onCaptureComplete: (filename: string) => void;
  onCancel: () => void;
}

export default function PlayerCameraCapture({ userId, onCaptureComplete, onCancel }: PlayerCameraCaptureProps) {
  const [dataUri, setDataUri] = useState<string | null>(null);
  const [facingMode, setFacingMode] = useState<any>(FACING_MODES.USER);
  ...
  const handleUpload = async () => {
    if (!dataUri || !userId) return;
    setIsUploading(true);

    try {
      const filename = `player_${userId}.png`;
      const res = await fetch('/api/upload_player_image', {
        method: 'POST',
        headers: {
    ...
    <div className="fixed inset-0 z-[100] bg-black bg-opacity-90 flex flex-col items-center justify-center">
      ...
      {!dataUri ? (
        ...
        <div className="flex flex-col items-center gap-6 w-full max-w-2xl px-4">
          ...
          <img src={dataUri} alt="Captured Profile" className="max-h-[60vh] max-w-full object-contain rounded border-2 border-slate-600 shadow-xl" />
          ...
          <div className="flex gap-4">
            ...
            <button
              ...
              className="bg-pink-600 hover:bg-pink-500 text-white px-6 py-3 rounded-lg font-bold flex items-center gap-2"
              disabled={isUploading}
            >
